...
fn collider_from_car_scene(scene_handle: &Handle<Scene>, scenes: &Assets<Scene>, meshes: &Assets<Mesh>) -> Option<CarColliders> {
    ...
    for entity in scene.world.iter_entities() {
        if let Some((name, mesh_handle, parent)) = entity.get_components::<(&Name, &Mesh3d, &ChildOf)>() {
...
fn get_collider_for_wheel(mesh: &Mesh, wheel_mesh_transform: &Transform) -> (Collider, Transform) {
    ...
    let transform = wheel_mesh_transform.with_translation(wheel_mesh_transform.translation + Vec3::from(aabb.center)).with_rotation(Quat::from_rotation_z(PI / 2.));
